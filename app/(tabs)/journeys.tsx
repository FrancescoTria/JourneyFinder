...
type StopRow = { id: number; nome?: string | null; lat?: number | null; lng?: number | null };
type PercorsoFermataRow = { id_percorso: number; id_fermata: number; ordine: number };
...
export default function JourneysScreen() {
  ...
  const [selectedJourney, setSelectedJourney] = useState<EnrichedJourney | null>(null);
  ...
  const fetchJourneys = useCallback(async () => {
    try {
      ...
      console.log(`[JourneysScreen] Recuperati ${journeyRows.length} viaggi dal database`);
      ...
      const enriched = (journeyRows as JourneyRow[]).map((journey) => {
        ...
        return {
          ...journey,
          route: journey.id_percorso ? routesMap.get(journey.id_percorso) : undefined,
  ...
  const routeCoordinates = useMemo(() => {
    if (!selectedJourney || !selectedJourney.stops || selectedJourney.stops.length === 0) {
      console.log(`[JourneysScreen] Nessuna fermata disponibile per il viaggio ${selectedJourney?.id}`);
      return [];
    }
    
    console.log(`[JourneysScreen] Viaggio ${selectedJourney.id}: ${selectedJourney.stops.length} fermate totali`);
    ...
    const coords = selectedJourney.stops
      .filter((stop) => {
        ...
        if (!hasLat || !hasLng) {
          console.log(`[JourneysScreen] Fermata "${stop?.nome}" senza coordinate valide: lat=${stop?.lat}, lng=${stop?.lng}`);
        }
    ...
    console.log(`[JourneysScreen] Coordinate percorso modale: ${coords.length} fermate con coordinate valide su ${selectedJourney.stops.length} totali`);
    if (coords.length > 0) {
      coords.forEach((coord, idx) => {
        console.log(`[JourneysScreen] Fermata ${idx + 1}: lat=${coord.latitude}, lng=${coord.longitude}`);
      });
    }
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <Text style={styles.title}>Viaggi disponibili</Text>
            <Text style={styles.subtitle}>
              {journeys.length > 0
                ? `${journeys.length} viaggi trovati. Tocca un viaggio per analizzare velocità media e traffico.`
                  ...
                  <>
                      ...
                      <View style={styles.chartContainer}>
                        <LineChart
                          data={{
                            ...
                            datasets: [
                              {
                                data: buildSpeedSeries(modalMetrics?.averageSpeed ?? 0),
                              },
                            ],
                          }}
                    ...
                    <View style={styles.mapWrapper}>
                      ...
                      {routeCoordinates.length === 0 ? (
                        <Text style={styles.emptyText}>
                          Nessuna fermata con coordinate valide per questo viaggio.
                        </Text>
                      ) : routeCoordinates.length === 1 ? (
                        <Text style={styles.emptyText}>
                          Solo una fermata con coordinate valide. Servono almeno 2 fermate per visualizzare il percorso.
                        </Text>
                        ...
                        <MapView
                          ...
                          onMapReady={() => {
                            ...
                            console.log('[JourneysScreen] Fermate totali nel viaggio:', selectedJourney.stops.length);
                            console.log('[JourneysScreen] Coordinate valide per la mappa:', routeCoordinates.length);
                            console.log('[JourneysScreen] Coordinate:', JSON.stringify(routeCoordinates));
                            console.log('[JourneysScreen] Regione mappa:', JSON.stringify(mapRegion));
                            
                            // Log delle fermate senza coordinate
                            const stopsWithoutCoords = selectedJourney.stops.filter(
                              (stop) => !stop || !Number.isFinite(Number(stop.lat)) || !Number.isFinite(Number(stop.lng))
                            );
                            if (stopsWithoutCoords.length > 0) {
                              console.log(`[JourneysScreen] ${stopsWithoutCoords.length} fermate senza coordinate valide:`, 
                                stopsWithoutCoords.map(s => s?.nome).join(', '));
                            }
                        ...
                        >
                          ...
                          <Marker
                            ...
                            title="Partenza"
                            description={selectedJourney.stops[0]?.nome ?? "Punto di partenza"}
                            pinColor="#22c55e"
                            identifier="start"
                            anchor={{ x: 0.5, y: 1 }}
                          />
                          ...
                          {routeCoordinates.length > 2 && routeCoordinates.slice(1, -1).map((coord, index) => {
                            ...
                            const correspondingStop = selectedJourney.stops.find(
                              (stop) => 
                                stop && 
                                Number.isFinite(Number(stop.lat)) && 
                                Number.isFinite(Number(stop.lng)) &&
                                Math.abs(Number(stop.lat) - coord.latitude) < 0.0001 &&
                                Math.abs(Number(stop.lng) - coord.longitude) < 0.0001
                            );
                            ...
                            return (
                              <Marker
                                ...
                                title={correspondingStop?.nome ?? `Fermata ${stopIndex + 1}`}
                          ...
                          {routeCoordinates.length > 1 && (
                            <Marker
                              ...
                              title="Arrivo"
                              description={selectedJourney.stops[selectedJourney.stops.length - 1]?.nome ?? "Punto di arrivo"}
                              pinColor="#ef4444"
                              identifier="end"
                              anchor={{ x: 0.5, y: 1 }}
                            />
